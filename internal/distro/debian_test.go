...
func TestParseAptListUpgradable(t *testing.T) {
	...
	if len(updates) != 3 { t.Fatalf("len = %d, want 3", len(updates)) }
	if updates[0].Name != "openssl" || !updates[0].Security { t.Errorf("[0] = %+v", updates[0]) }
	if updates[0].CurrentVersion != "3.0.2-0ubuntu1.15" { t.Errorf("current = %q", updates[0].CurrentVersion) }
	if updates[0].AvailableVersion != "3.0.2-0ubuntu1.16" { t.Errorf("available = %q", updates[0].AvailableVersion) }
	if updates[2].Name != "curl" || updates[2].Security { t.Errorf("[2] should NOT be security: %+v", updates[2]) }
	for _, u := range updates {
		if u.Source != "apt" { t.Errorf("Source = %q", u.Source) }
	}
}
...
func TestDebianUpgradeArgsAll(t *testing.T) {
	...
	if len(args) != 3 || args[1] != "upgrade" { t.Errorf("args = %v", args) }
}

func TestDebianUpgradeArgsSpecific(t *testing.T) {
	args := debianUpgradeArgs(PatchFilter{IncludePackages: []string{"openssl", "curl"}})
	if len(args) != 6 || args[1] != "install" || args[2] != "--only-upgrade" { t.Errorf("args = %v", args) }
}
