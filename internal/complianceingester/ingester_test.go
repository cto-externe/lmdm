...
func TestIntegrationComplianceIngesterPersists(t *testing.T) {
	if testing.Short() {
	...
	if err := repo.Insert(ctx, &devices.Device{
		...
		Hostname: "PC-COMP", AgentPubkeyEd25519: []byte("ed-c"), AgentPubkeyMLDSA: []byte("ml-c"),
	}); err != nil {
		t.Fatal(err)
	}
	...
	report := &lmdmv1.ComplianceReport{
		...
		OverallStatus: lmdmv1.ComplianceStatus_COMPLIANCE_STATUS_COMPLIANT,
		TotalChecks:   4,
		PassedChecks:  4,
		FailedChecks:  0,
	}
	data, _ := proto.Marshal(report)
	if err := plainNC.Publish("fleet.agent."+deviceID.String()+".compliance", data); err != nil {
		t.Fatal(err)
	}
	_ = plainNC.Flush()
	...
	for time.Now().Before(deadline) {
		...
		err := pool.QueryRow(ctx,
			`SELECT overall_status FROM compliance_reports WHERE device_id = $1`, deviceID,
		).Scan(&status)
	...
	t.Fatal("compliance_reports not populated within 5s")
}
