...
func TestIntegrationIngesterUpdatesLastSeen(t *testing.T) {
	if testing.Short() {
	...
	if err := repo.Insert(ctx, &devices.Device{
		...
		Hostname:           "PC-INGEST",
		AgentPubkeyEd25519: []byte("ed-ingest"),
		AgentPubkeyMLDSA:   []byte("ml-ingest"),
	}); err != nil {
		t.Fatal(err)
	}
	...
	subject := "fleet.agent." + deviceID.String() + ".status"
	...
	for time.Now().Before(deadline) {
		got, err := repo.FindByID(ctx, tenantID, deviceID)
		if err == nil && got.LastSeen != nil && got.AgentVersion != nil && *got.AgentVersion == "0.1.0-ingest" {
			return
		}
		time.Sleep(100 * time.Millisecond)
	}
	t.Fatal("device.last_seen / agent_version was not updated within 5s")
}
